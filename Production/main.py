...
def draw_char_scaled(oled, char, x, y, scale_x=1, scale_y=2):
            ...
            if (bits >> (7-col)) & 1:
                for dy in range(scale_y):
                    for dx in range(scale_x):
                        oled.pixel(x + col*scale_x + dx, y + row*scale_y + dy, 1)

def draw_text_scaled_fit(oled, text, y=8, max_width=128, scale_y=2, margin=2):
    n_chars = len(text)
    if n_chars == 0:
        return
    scale_x = (max_width - 2*margin) // (n_chars * 8)
    if scale_x < 1:
        scale_x = 1
    x = margin
    for c in text:
        draw_char_scaled(oled, c, x, y, scale_x, scale_y)
        x += 8 * scale_x
...
def update_oled(oled):
    ...
    draw_text_scaled_fit(oled, mode_names[current_layer], y=8, max_width=128, scale_y=2, margin=2)
    oled.show()
